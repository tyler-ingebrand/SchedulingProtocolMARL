...
class RecordVideo(gym.Wrapper):
    ...
    def __init__(
        ...
        self.is_multi_agent = isinstance(env, ParallelEnv)
        self.episode_id = 0
    ...
    def step(self, action):
        ...
        if not (self.terminated or self.truncated):
            ...
            self.step_id += 1
            if not self.is_vector_env and not self.is_multi_agent:
                ...
                if all(terminateds.values()) or all(truncateds.values()):
                    self.episode_id += 1
                    self.terminated = all(terminateds)
                    self.truncated = all(truncateds)
    ...
    def render(self, *args, **kwargs):
        ...
        if len(self.video_recorder.render_history) > 0:
            ...
            if self.recording:
                return None #  self.video_recorder.last_frame
            else:
                return super().render(*args, **kwargs)
