...
def run_experiment(protocol,
    ...
    # set up environment
    env = DroneGameEnvironment(number_agents = number_agents, size = game_size, timeout = timeout, render_mode = None, protocol = protocol, turn_length = turn_length)
    ...
    def update_memory_CSMA_CD(state, memory):
        ...
        me_in_hallway = env.grid_size[0] // 2 - 1 <= state[0] <= env.grid_size[0] // 2 + 1
    ...
    def reward_empty_hallway(s, a, ns, info):
        return  1.0 if (ns[0], ns[1]) in env.job_stations and ns[2] > 0 else 0.0


    def reward_occupied_hallway(s, a, ns, info):
        if (ns[0], ns[1]) in env.recharging_stations:
            return -1.0
        else:
            return  1.0 if (ns[0], ns[1]) in env.job_stations and ns[2] > 0 else 0.0
        ...
        agent = Multi_Agent({
                        agent_name: Compositional_Agent(
                            agents=[State_Action_Transforming_Agent(agent=Tabular_Q_Agent(tabular_q_state_space, action_space, hook=Do_Nothing_Hook(),
                                                                                        exploration_type=Tabular_Q_Agent.ExplorationType.Boltzmann, device=device),
                            ...
                            reward_functions=[reward_empty_hallway,reward_occupied_hallway],
    ...
    # set file names
    file_name = f"results/drone_game_{number_agents}_agents_{protocol}_seed_{seed}.pkl"
    video_file_name_prefix = f"drone_game_{number_agents}_agents_{protocol}_seed_{seed}"


    # run experiment
    run(env, agent, number_steps, train = True, show_progress=True)
    if render: # TODO fix this
        agent.plot()
    ...
    # render
    if save_video:
        render_env = DroneGameEnvironment(number_agents = number_agents, size = game_size, render_mode="rgb_array")
    ...
    if render:
        render_env = DroneGameEnvironment(number_agents = number_agents, size = game_size, render_mode="human")
        run(render_env, agent, number_testing_steps, train = False, show_progress=True, verbose=True)

if __name__ == "__main__":
    run_experiment(protocol="STR", seed=0, number_steps=1000_000, number_agents=10, device="cpu", render=False, save_video=True)
